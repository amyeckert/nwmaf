...
class GP_Limit_Choices extends GWPerk {
	...
	public function init() {
		...
		$this->add_tooltip( $this->key( 'enableLimits' ), __( '<h6>Limit Choices</h6> Limit the number of times a choice can be selected across all submitted entries. When the limit is reached the choice will become unavailable.', 'gwlimitchoices' ) );
		...
		add_filter( 'gform_field_choice_markup_pre_render', array( $this, 'filter_removed_choices' ), 10, 4 );
	...
	private function register_scripts() {
		...
		wp_localize_script( $this->key( 'admin' ), 'GPLCAdminData', array(
			...
			'enableCheckboxLegacyMarkup' => $this->get_legacy_enable_checkbox_markup(),
		) );
	...
	/**
	 * Outputs markup for the enable limit checkbox for GF 2.4 and older.
	 *
	 * @see self::field_settings() for GF 2.5+.
	 * @see self::register_scripts() consumes this markup.
	 *
	 * @return string
	 */
	public function get_legacy_enable_checkbox_markup() {
		return sprintf( '
	...
	/**
	 * Add the custom settings for Limit Choices to the fields general tab.
	 *
	 * Note, for legacy UI (GF 2.4), we use self::get_legacy_enable_checkbox_markup()
	 * along with JavaScript to inject the checkbox into the UI.
	 *
	 * The column containing inputs for the limit for each choice is still handled via JavaScript.
	 *
	 * To make this show in the flyout, we utilize the data-js and data-type attributes. We are then
	 * responsible for showing the field much like other field settings added by perks.
	 *
	 * @see self::get_legacy_enable_checkbox_markup()
	 *
	...
	public function field_settings( $position, $form_id ) {
		...
		printf( '
				<li class="enable_limits_setting field_setting" data-js="choices-ui-setting" data-type="option">
					<input type="checkbox" onclick="SetFieldProperty( \'%s\', this.checked ); GPLCAdmin.toggleEnableLimits();" id="field_choice_limits_enabled">
					<label class="inline" for="field_choice_limits_enabled">%s</label>
					' . gform_tooltip( $this->key( 'enableLimits' ), '', true ) . '
				</li>',
			$this->key( 'enableLimits' ),
			__( 'Enable Limits', 'gp-limit-choices' )
		);
	...
	public function filter_removed_choices( $choice_markup, $choice, $field, $value ) {
		if ( rgar( $choice, 'isRemoved' ) ) {
			return '';
		}
	...
	public function apply_choice_limits( $choices, $field, $form, $remove = true ) {
		...
		// Set to true by default and set to false if any choice not removed is present.
		$is_choiceless = true;
		...
		/*
		 * Filter out removed choices for any fields outside of radios and checkboxes due to them not supporting
		 * the `gform_field_choice_markup_pre_render` filter hook.
		 */
		if ( ! in_array( $field->get_input_type(), array( 'checkbox', 'radio' ) ) ) {
			$choices = array_filter( $choices, function ( $choice ) {
				return ! rgar( $choice, 'isRemoved' );
			} );
		}
		...
		foreach ( $field['choices'] as $choice_id => $choice ) {
			...
			switch ( $field_type ) {
				...
				default:
					...
					$content = str_replace( $search, $replace, $content );
					$content = preg_replace( "/(<input class=\')(.+?\'.+?$search.+?\/>)/", '$1gwlc-disabled $2', $content );
					break;
			}
		}
	...
	public function is_field_exhausted( $field ) {
		...
		if ( ! isset( $this->choiceless[ $field->formId ] ) ) {
			$this->choiceless[ $field->formId ] = array();
			$field->choices                     = $this->apply_choice_limits( $field->choices, $field, self::$current_form );
		}

		return in_array( $field->id, $this->choiceless[ $field->formId ] );
	}
