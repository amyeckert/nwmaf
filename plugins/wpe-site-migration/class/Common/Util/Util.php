...
class Util
{
    ...
    {
        $state_data = Persistence::getStateData();
        if (!array_key_exists('source_url', $state_data)) {
            return false;
        }

        $site_details = $state_data['site_details'];
        if ($state_data['intent'] === 'push') {
            return $site_details['local']['site_url'] === $state_data['source_url'];
        }

        if ($state_data['intent'] === 'pull') {
            return $site_details['remote']['site_url'] === $state_data['source_url'];
        }
