...
class GF_Field_PayPal extends GF_Field_CreditCard {
	...
	public function get_field_input( $form, $value = array(), $entry = null ) {
		...
		if ( ! gf_ppcp()->initialize_api() ) {
			if ( ! $is_admin ) {
				return sprintf( esc_html__( '%sPlease check your PayPal Checkout Add-On settings. Your account is not connected yet.%s' ), '<div class="gfield_description validation_message gfield_validation_message">', '</div>' );
			} else {
		...
		if ( ! $is_admin && ! gf_ppcp()->has_feed( $form['id'] ) ) {
			return sprintf( esc_html__( '%sPlease check if you have activated a PayPal Checkout feed for your form.%s' ), '<div class="gfield_description validation_message gfield_validation_message">', '</div>' );
		}
